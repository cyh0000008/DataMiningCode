...
class VseActivationCountMetric(BaseMetric):
    name = "vse_activation_count"
    category = "MPI"
    result_mode = "event_count"
    required_fields = ("vse_active",)
    active_values = {1}

    def evaluate(self, frame: SignalFrame) -> MetricResult:
        mask = enum_mask(frame.col("vse_active"), self.active_values)
...
def create_metric() -> BaseMetric:
    return VseActivationCountMetric()
