...
def _core_process(comprehensive_frames: List[Dict]) -> Tuple[bool, List[str], Dict]:  # 修复7：改为Any，兼容字符串/字典
    ...
    for i,segment in enumerate(risk_segments):
        ...
        result_values = _calc_segment_max_vel_fluct(segment)
        if result_values:
            if _check_max_vel_fluct_valid(result_values) is not True:
                ...
                check_result = True
                jerk_values.append(f"[Vel Fluct Dangerous! setspeed_kph:{result_values['set_speed']} , max_vel_diff:{result_values['max_vel_diff']:.6f}, min_vel_kph:{result_values['min_vel_chassis']:.6f}, min_vel_time_ms:{result_values['min_vel_chassis_time']:.3f}, max_vel_kph:{result_values['max_vel_chassis']:.6f}, max_vel_time_ms:{result_values['max_vel_chassis_time']:.3f}]")
            else:
                jerk_values.append(f"[setspeed_kph:{result_values['set_speed']} , max_vel_diff:{result_values['max_vel_diff']:.6f}, min_vel_kph:{result_values['min_vel_chassis']:.6f}, min_vel_time_ms:{result_values['min_vel_chassis_time']:.3f}, max_vel_kph:{result_values['max_vel_chassis']:.6f}, max_vel_time_ms:{result_values['max_vel_chassis_time']:.3f}]")
        else:
            jerk_values.append(f"当前段落算不出jerk")

    return (check_result, jerk_values, case_statistics)

def _check_max_vel_fluct_valid(result_values: Optional[Dict]) -> bool:
    C_fluct_upper_bound = 1

    if result_values["vel_chassis_diff"] <= C_fluct_upper_bound:
        return True
    else:
        return False
...
def _cal_severity_score(result_values: Optional[Dict]) -> float:
    ...
    if result_values["vel_chassis_diff"] <= C_fluct_upper_bound:
        ...
        return abs((result_values["vel_chassis_diff"] - C_fluct_upper_bound)/C_cal_base)*100.0
...
def _calc_segment_max_vel_fluct(segment: List[Dict]) -> Optional[Dict]:
    """
    ...
    # 筛选稳速段内的底盘速度数据
    steady_vel_dashboard = []
    for vel, ts in vel_dashboard_with_time:
        try:
            # 确保所有参与比较的变量都是float类型
            ts_float = float(ts) if ts is not None else 0.0
            vel_float = float(vel) if vel is not None else 0.0
            if start_ts <= ts_float <= end_ts:
                steady_vel_dashboard.append((vel_float, ts_float))
        except (ValueError, TypeError):
            continue
    
    # 初始化返回字段
    max_vel_chassis = None
    max_vel_chassis_time = None
    min_vel_chassis = None
    min_vel_chassis_time = None
    vel_chassis_diff = None  # 最大值与最小值的差值
    max_vel_diff = None  # 最大值与SetSpeed的差值
    
    if steady_vel_dashboard:
        # 找到最大底盘速度及对应时间
        max_vel_chassis, max_vel_chassis_time = max(steady_vel_dashboard, key=lambda x: x[0])
        # 找到最小底盘速度及对应时间
        min_vel_chassis, min_vel_chassis_time = min(steady_vel_dashboard, key=lambda x: x[0])
        # 计算差值
        vel_chassis_diff = max_vel_chassis - min_vel_chassis
        max_vel_diff = max_vel_chassis - min_vel_chassis

    # 返回结果（替换为新的统计字段）
    return {
        "max_vel_chassis": max_vel_chassis,
        "max_vel_chassis_time": max_vel_chassis_time,
        "min_vel_chassis": min_vel_chassis,
        "min_vel_chassis_time": min_vel_chassis_time,
        "vel_chassis_diff": vel_chassis_diff,
