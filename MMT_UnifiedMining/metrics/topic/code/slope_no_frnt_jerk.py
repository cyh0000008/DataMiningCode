...
def _core_process(comprehensive_frames: List[Dict]) -> Tuple[bool, List[str], Dict]:  # 修复7：改为Any，兼容字符串/字典
    ...
    for i,segment in enumerate(risk_segments):
        ...
        result_values = _calc_segment_max_jerk_val(segment)
        if result_values:
            if _check_max_jerk_val_valid(result_values) is not True:
                ...
                check_result = True
                jerk_values.append(f"[Max Jerk Dangerous! setspeed_kph:{result_values['set_speed']} , max_jerk_val:{result_values['max_jerk']:.6f}, max_jerk_time:{result_values['max_jerk_time']:.6f}]")
            else:
                jerk_values.append(f"[setspeed_kph:{result_values['set_speed']} , max_jerk_val:{result_values['max_jerk']:.6f}, max_jerk_time:{result_values['max_jerk_time']:.6f}]")
        else:
            jerk_values.append(f"当前段落无有效jerk")

    return (check_result, jerk_values, case_statistics)

def _check_max_jerk_val_valid(result_values: Optional[Dict]) -> bool:
    C_jerk_val_upper_bound = 1

    if abs(result_values["max_jerk"]) <= C_jerk_val_upper_bound:
        return True
    else:
        return False
...
def _cal_severity_score(result_values: Optional[Dict]) -> float:
    ...
    if abs(result_values["max_jerk"]) <= C_jerk_val_upper_bound:
        ...
        exceed_amount = abs(result_values["max_jerk"]) - C_jerk_val_upper_bound
...
def _calc_segment_max_jerk_val(segment: List[Dict]) -> Optional[Dict]:
    """
    ...
    for frame in segment:
        ...
        if matched_chassis and "accleration_on_wheel" in matched_chassis:
            ...
            chassis_ts = matched_chassis["timestamp_ms"]
            ts = frame.get("unp_timestamp_ms")
            ...
            if acc_chassis is not None and ts is not None:
                ...
                if final_acc is not None:
                    acc_chassis_with_time.append((final_acc, slope_acc, acc_chassis, ts, chassis_ts))
    ...
    # 步骤4：统计稳速段内的底盘加速度数据 - 核心修复：时间比较防护
    steady_acc_chassis = []
    for final_acc, acc_slope, acc, ts, chassis_ts in acc_chassis_with_time:
        try:
            # 确保所有参与比较的变量都是数值
            ts_float = float(ts) if ts is not None else 0.0
            final_acc_float = float(final_acc) if final_acc is not None else 0.0
            chassis_ts_float = float(chassis_ts) if chassis_ts is not None else 0.0
            
            if start_ts <= ts_float <= end_ts:
                steady_acc_chassis.append((final_acc_float, chassis_ts_float))
        except (ValueError, TypeError):
            continue
    
    if not steady_acc_chassis:
        print("警告：稳速段内无有效加速度数据")
        return {
            "max_jerk": None,
            "max_jerk_time": None,
            "set_speed": set_speed
    ...
    # 计算最大jerk值
    max_jerk_item = calculate_max_smoothed_jerk_with_time(steady_acc_chassis)
