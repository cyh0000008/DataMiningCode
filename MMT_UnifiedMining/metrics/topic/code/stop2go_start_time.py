...
def _core_process(comprehensive_frames: List[Dict]) -> Tuple[bool, List[str], Dict]:  # 修复7：改为Any，兼容字符串/字典
    ...
    for i,segment in enumerate(risk_segments):
        ...
        result_values = _calc_segment_start_time(segment)
        if result_values:
            if _check_start_time_valid(result_values) is not True:
                ...
                check_result = True
                jerk_values.append(
                    f"[Stop2Go Start Time Dangerous! "
                    f"车动起来耗时:{result_values['move_time']:.3f} , "
                    f"起步耗时:{result_values['reach_target_time']:.3f}, "
                    f"t0_time_ms:{result_values['t0_time_ms']:.3f}, "
                    f"t1_time_ms:{result_values['t1_time_ms']:.3f}, "
                    f"t2_time_ms:{result_values['t2_time_ms']:.3f}, "
                    f"target_speed:{result_values['target_speed']:.3f}, "
                    f"max_speed:{result_values['max_vehicle_speed']:.3f}]"
                )
            else:
                jerk_values.append(
                    f"车动起来耗时:{result_values['move_time']:.3f} , "
            ...
            jerk_values.append(f"当前段落无有效起步时间")

    return (check_result, jerk_values, case_statistics)
    
def _check_start_time_valid(result_values: Optional[Dict]) -> bool:
    C_max_time = 3.5
    C_min_time = 1.0

    start_time = result_values["reach_target_time"]

    if start_time <= C_max_time and start_time >= C_min_time:
        return True
    else:
        return False
...
def _cal_severity_score(result_values: Optional[Dict]) -> float:
    ...
    if start_time <= C_max_time and start_time >= C_min_time:
        ...
        if start_time < C_min_time:
            exceed_amount = C_min_time - start_time
        else:
            exceed_amount = start_time - C_max_time
...
def _calc_segment_start_time(segment: List[Dict]) -> Optional[Dict]:
    """
    计算起步阶段的关键时间节点：
    1. 找到accleration_on_wheel+slope_acc第一次>0.1的时间t0（ms）
    2. 找到vehicle_speed第一次>0.2的时间t1（ms）
    3. 找到vehicle_speed第一次>target_speed的时间t2（ms）
    4. 返回t1-t0(秒)、t2-t1(秒)、target_speed、最大车速
    ...
    K_target_speed = 3.33
    target_speed = float(K_target_speed)  # 确保目标速度为数值类型

    # ========== 步骤2：初始化变量（全量None防护） ==========
    valid_data: List[Dict] = []  # 存储有效帧的(合成加速度, 车速, 底盘时间)
    all_vehicle_speeds: List[float] = []  # 存储所有有效车速
    ...
    for frame_idx, frame in enumerate(segment):
        ...
        chassis_time = matched_chassis.get("timestamp_ms") or frame.get("unp_timestamp_ms")  # 双重时间戳兜底
        ...
        try:
            ...
            print(f"警告：第{frame_idx}帧数据类型错误（{str(e)}），无法转换为浮点数，跳过")
            continue

        # ========== 步骤5：计算合成加速度，存储有效数据 ==========
        combined_acc = accleration_on_wheel + slope_acc  # 合成加速度（原代码注释与逻辑不一致，修正）
        
        valid_frame = {
            "combined_acc": combined_acc,          # 合成加速度（轮端+坡度）
            "vehicle_speed": vehicle_speed,        # 车辆速度
            "chassis_time": chassis_time           # 底盘时间戳（ms）
        }
    ...
    if not valid_data or not isinstance(valid_data, list):
        ...
        return None
    if not all_vehicle_speeds:
        all_vehicle_speeds = [0.0]  # 兜底，避免后续max()报错

    # ========== 步骤7：按时间排序（强化None防护） ==========
    try:
        # 排序前校验时间字段有效性，避免None参与排序
        valid_data_sorted = sorted(
            [d for d in valid_data if isinstance(d.get("chassis_time"), (int, float))],
            key=lambda x: x["chassis_time"]
        )
        if not valid_data_sorted:
            print("警告：无有效时间戳数据，无法排序")
            return None
    except (ValueError, TypeError):
        print("警告：时间戳数据异常，无法完成排序")
        return None

    # ========== 步骤8：查找关键时间节点（强化None/条件防护） ==========
    t0 = None  # 合成加速度首次>0.1的时间（修正原代码0.2的错误，匹配注释）
    t1 = None  # 车速首次>0.2的时间
    t2 = None  # 车速首次>target_speed的时间

    # 遍历排序后的有效数据，查找首次满足条件的时间
    for data in valid_data_sorted:
        # 防护：确保数据字段有效
        curr_acc = data.get("combined_acc", 0.0)
        curr_speed = data.get("vehicle_speed", 0.0)
        curr_time = data.get("chassis_time", 0.0)
        
        # 类型校验，避免None参与数值比较
        if not isinstance(curr_acc, (int, float)) or not isinstance(curr_time, (int, float)):
            continue
        
        # 查找t0：合成加速度首次>0.1（修正原代码0.2，匹配函数注释）
        if t0 is None and curr_acc > 0.1:
            t0 = curr_time
        
        # 查找t1：车速首次>0.2（类型防护）
        if t1 is None and isinstance(curr_speed, (int, float)) and curr_speed > 0.2:
            t1 = curr_time
        
        # 查找t2：车速首次>目标速度（类型防护）
        if t2 is None and isinstance(curr_speed, (int, float)) and curr_speed > target_speed:
            t2 = curr_time
    ...
    # ========== 步骤9：处理时间节点缺失的情况（优化兜底逻辑） ==========
    missing_nodes = []
    if t0 is None:
        missing_nodes.append("t0（合成加速度首次>0.1）")
    if t1 is None:
        missing_nodes.append("t1（车速首次>0.2）")
    if t2 is None:
        missing_nodes.append("t2（车速首次>目标速度）")
    
    if missing_nodes:
        print(f"警告：未找到以下时间节点：{', '.join(missing_nodes)}")
        return None

    # ========== 步骤10：计算时间差（全量数值防护） ==========
    # 强制转换为浮点数，避免None参与运算
    t0 = float(t0) if isinstance(t0, (int, float)) else 0.0
    t1 = float(t1) if isinstance(t1, (int, float)) else 0.0
    t2 = float(t2) if isinstance(t2, (int, float)) else 0.0

    # 计算时间差（转换为秒），确保非负
    t1_minus_t0 = max((t1 - t0) / 1000.0, 0.0)  # 避免负数时间差
    t2_minus_t1 = max((t2 - t1) / 1000.0, 0.0)
    ...
    try:
        ...
        print("警告：无法计算最大车速，使用默认值0.0")

    # ========== 步骤12：组装返回结果（最终None防护） ==========
    result = {
        "move_time": round(t1_minus_t0, 3),          # t1-t0（秒，保留3位小数）
        "reach_target_time": round(t2_minus_t1, 3),  # t2-t1（秒，保留3位小数）
        "t0_time_ms": round(t0, 3),
        "t1_time_ms": round(t1, 3),
        "t2_time_ms": round(t2, 3),
        "target_speed": round(target_speed, 3),      # 目标速度（保留3位小数）
        "max_vehicle_speed": round(max_vehicle_speed, 3)  # 最大车速（保留3位小数）
    }
