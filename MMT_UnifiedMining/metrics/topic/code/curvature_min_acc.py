...
def _core_process(comprehensive_frames: List[Dict]) -> Tuple[bool, List[str], Dict]:  # 修复7：改为Any，兼容字符串/字典
    ...
    for i,segment in enumerate(risk_segments):
        ...
        result_values = _calc_segment_curv_acc(segment)
        if result_values:
            if _check_min_curv_acc_valid(result_values) is not True:
                ...
                check_result = True
                jerk_values.append(f"[Min Curv Dec Dangerous! setspeed_mps:{result_values['set_speed_at_min_speed']} , min_acc:{result_values['min_deceleration']:.6f}, min_curv_radius:{result_values['min_curvature_radius']:.2f}, min_decel_time:{result_values['min_decel_time']:.3f}]")
            else:
                jerk_values.append(f"[setspeed_mps:{result_values['set_speed_at_min_speed']} , min_acc:{result_values['min_deceleration']:.6f}, min_curv_radius:{result_values['min_curvature_radius']:.2f}, min_decel_time:{result_values['min_decel_time']:.3f}]")
        else:
            jerk_values.append(f"当前段落无有效弯道减速度")

    return (check_result, jerk_values, case_statistics)
    
def _check_min_curv_acc_valid(result_values: Optional[Dict]) -> bool:
    C_min_acc = -2

    min_acc = result_values["min_deceleration"]

    if min_acc >= C_min_acc:
        return True
    else:
        return False
...
def _cal_severity_score(result_values: Optional[Dict]) -> float:
    ...
    if min_acc >= C_min_acc:
        ...
        return abs((min_acc - C_min_acc)/C_cal_base)*100.0
...
def _calc_segment_curv_acc(segment: List[Dict]) -> Optional[Dict]:
    """
    ...
    # print(f"\n===== 转折点({turning_point_idx})向前到第一帧(0)的每帧减速度 =====")
    # # 从turning_point_idx递减遍历到0，逐个打印索引和减速度
    # for idx in range(turning_point_idx, -1, -1):
    #     frame = speed_valid_frames[idx]
    #     decel = frame["deceleration"]
    #     # 标注是否满足"逐渐降低"条件，方便对比
    #     print(f"  speed_valid_frames索引{idx}：deceleration = {decel:.4f} | slope_acc: {frame['slope_acc']} | accleration_on_wheel: {frame['accleration_on_wheel']}")
    ...
    while current_trace_idx >= 0:
        ...
        if is_valid:
            ...
            # print(f"  索引{current_trace_idx}不满足条件，连续不满足帧数：{consecutive_invalid_count}")
            if consecutive_invalid_count >= MAX_CONSECUTIVE_INVALID:
                # print(f"  连续{MAX_CONSECUTIVE_INVALID}帧不满足条件，终止溯源")
                break
            # 未达到阈值，继续向前检查下一个帧
            current_trace_idx -= 1
    ...
    min_decel_frame = min(valid_traceback_frames, key=lambda x: x["deceleration"])

    # # ========== 新增：打印溯源区间的始末点信息 ==========
    # # 溯源区间末点：转折点（索引最大的满足条件的帧）
    # trace_end_idx = valid_traceback_indices[0]
    # trace_end_frame = speed_valid_frames[trace_end_idx]
    # # 溯源区间始点：最前端满足条件的帧（索引最小的满足条件的帧）
    # trace_start_idx = valid_traceback_indices[-1]
    # trace_start_frame = speed_valid_frames[trace_start_idx]

    # # 打印始末点详细信息
    # print(f"\n===== 溯源区间信息 =====")
    # print(f"转折点索引（溯源起始位置）：{turning_point_idx}")
    # print(f"溯源区间末点（靠近转折点）：")
    # print(f"  - speed_valid_frames索引：{trace_end_idx}")
    # print(f"  - 减速度值：{trace_end_frame['deceleration']:.4f}")
    # print(f"  - 车速：{trace_end_frame['vehicle_speed']:.2f}")
    # print(f"溯源区间始点（最前端满足条件）：")
    # print(f"  - speed_valid_frames索引：{trace_start_idx}")
    # print(f"  - 减速度值：{trace_start_frame['deceleration']:.4f}")
    # print(f"  - 车速：{trace_start_frame['vehicle_speed']:.2f}")
    # print(f"溯源区间总帧数：{len(valid_traceback_indices)}")
    # print(f"找到的最小减速度帧索引：{speed_valid_frames.index(min_decel_frame)}，减速度值：{min_decel_frame['deceleration']:.4f}")
    # print(f"=========================\n")
    ...
    result = {
        ...
        # 新增：减速度逐渐降低过程的起点（最小deceleration）
        "min_deceleration": min_decel_frame["deceleration"],
        "min_decel_vehicle_speed": min_decel_frame["vehicle_speed"],
        "min_decel_time": min_decel_frame["chassis_time"],
        "slope_acc_at_min_decel": min_decel_frame["slope_acc"],
        "accleration_on_wheel_at_min_decel": min_decel_frame["accleration_on_wheel"]
    }
