...
def __run_haoran_data_run__(folder_context: dict[str, Any]) -> tuple[bool, list[str], dict[str, Any]]:
    ...
    result = analyze_acc_jerkiness_after_override_release(session)
    ...
    result_lines.append(
        f"[ACC Jerkiness After Override Release] file={Path(session.primary_h5_file).name}, "
    ...
    for scene in result["scenes"]:
        result_lines.append(
            f"[Scene release={scene['release_timestamp']:.3f}, start={scene['start_timestamp']:.3f}, "
...
def analyze_acc_jerkiness_after_override_release(
    ...
    **jerkiness_options: Any,
    ...
    if len(axis) < 3:
        return _empty_scene_result(start_timestamp, end_timestamp, post_override_window_seconds)
    ...
    jerkiness_result = _ACC_JERKINESS.evaluate_acc_jerkiness_from_axis(
        ...
        metric_name="acc_jerkiness_after_override_release",
        method="post_override_release_torque_gated_jerkiness",
        **jerkiness_options,
    )
...
def _empty_scene_result(start_timestamp: float, end_timestamp: float, post_override_window_seconds: float) -> dict[str, object]:
    return {
        "metric_name": "acc_jerkiness_after_override_release",
        "all_case_count": 0,
        "bad_case_count": 0,
        ...
        "start_timestamp": start_timestamp,
        "end_timestamp": end_timestamp,
        "parameters": {"post_override_window_seconds": post_override_window_seconds},
        "scenes": [],
        "events": [],
    }
