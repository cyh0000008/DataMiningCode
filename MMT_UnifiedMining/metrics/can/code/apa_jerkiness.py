...
def __run_haoran_data_run__(folder_context: dict[str, Any]) -> tuple[bool, list[str], dict[str, Any]]:
    ...
    result = count_apa_jerkiness_events(session)
    ...
    result_lines.append(
        f"[APA Jerkiness] file={Path(session.primary_h5_file).name}, event_count={event_count}, "
...
def count_apa_jerkiness_events(
    ...
    apa_status_values = session.sample_signal_to_axis(
        SIGNAL_KEYS["apa_status"], axis, method="nearest", start_timestamp=start_timestamp, end_timestamp=end_timestamp
    )

    valid_mask = [
        _matches_enum(apa_type, APA_TYPE_VALUES, {"1"})
        and _matches_enum(apa_status, APA_STATUS_VALUES, {"5", "6", "apa guidance", "guidance", "active", "apa finish", "finish"})
        for apa_type, apa_status in zip(apa_type_values, apa_status_values)
    ]
    ...
    events = _filter_events_by_reversal_pattern(
        events=events,
        timestamps=candidate_timestamps,
        smoothed_acceleration=smoothed_acceleration[1:],
