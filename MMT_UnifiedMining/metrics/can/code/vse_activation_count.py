...
METRIC_CATEGORY = "MPI"
VSE_ACTIVE_SIGNAL = "PPEI_Chassis_General_Data_3_0AC_M.IVSEAct"
...
def __run_haoran_data_run__(folder_context: dict[str, Any]) -> tuple[bool, list[str], dict[str, Any]]:
    ...
    result = analyze_vse_activation_count(session)
    ...
    result_lines.append(
        f"[VSE Activation Count] file={Path(session.primary_h5_file).name}, all_case_count={all_case_count}, "
...
def analyze_vse_activation_count(
    ...
    axis, vse_active_raw = session.get_signal_slice(SIGNAL_KEYS["vse_active"], start_timestamp, end_timestamp)
    if len(axis) < 2:
        return _empty_result(start_timestamp, end_timestamp)

    vse_active_mask = [
        _matches_enum(value, VSE_ACTIVE_VALUES, {"1", "true", "active"})
        for value in vse_active_raw
    ]

    scenes = []
    severity_score = 0.0
    for segment in mask_to_segments(axis, vse_active_mask):
    ...
    return {
        "metric_name": "vse_activation_count",
        ...
        "parameters": {
            "vse_active_values": sorted(VSE_ACTIVE_VALUES),
        },
        "scenes": scenes,
    }
