...
还可以配置“原始报告额外保存目录”：挖掘成功后会把本次原始 XLSX 复制到该目录，
并按 Target 表头的 `车型-日期-版本.xlsx` 重命名。为确保归档文件名与最终表头一致，
启用此项时必须填写 Target 的车型、日期和软件版本；同名文件会追加序号而不会覆盖。
...
随 MMT 参数预设一起保存；也支持按相同规则将本次原始 XLSX 额外归档并重命名。
